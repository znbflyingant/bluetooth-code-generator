...
function generateClientServiceCode(generateBoth, reqClassName, rspClassName, baseName) {
    ...
    let code = `package ${servicePackage}.aires

import com.tempolor.aimusic.ble.client.ResponseResultCodeEnum
import com.tempolor.aimusic.ble.client.SendDataInChunksHelper
import com.tempolor.aimusic.ble.client.SendDataWithResponseHelper`;
    
    // 添加请求类的导入
    if (generateBoth) {
        if (reqClassName) {
            code += `\nimport com.tempolor.aimusic.proto.aires.${reqClassName}`;
        }
        if (rspClassName) {
            code += `\nimport com.tempolor.aimusic.proto.aires.${rspClassName}`;
        }
    } else {
        if (reqClassName) {
            code += `\nimport com.tempolor.aimusic.proto.aires.${reqClassName}`;
    ...
    code += `\n\nobject ${clientServiceName} {`;
    
    // 生成方法
    if (generateBoth) {
        // 生成Req和Rsp对应的方法
        if (reqClassName && includeInChunk) {
            const methodName = getServiceMethodName(baseName);
            const paramName = baseName.toLowerCase() + 'Req';
            code += `\n    suspend fun reqInChunk${methodName}(${paramName}: ${reqClassName}): Boolean {\n`;
        ...
        if (reqClassName && includeWithResponse) {
            const methodName = getServiceMethodName(baseName);
            const paramName = baseName.toLowerCase() + 'Req';
            code += `\n    suspend fun reqWithResponse${methodName}(${paramName}: ${reqClassName}): Boolean {\n`;
...
function generateServerServiceCode(generateBoth, reqClassName, rspClassName, baseName) {
    ...
    let code = `package ${servicePackage}.aires.debug

import com.tempolor.aimusic.ble.client.ResponseResultCodeEnum
import com.tempolor.aimusic.ble.client.SendDataInChunksHelper
import com.tempolor.aimusic.ble.client.SendDataWithResponseHelper
import com.tempolor.aimusic.ble.service.BleSendDataHelper`;
    ...
    if (generateBoth) {
        ...
        if (reqClassName && includeServerSupReq) {
            const methodName = getServiceMethodName(baseName);
            const paramName = baseName.toLowerCase() + 'Req';
            code += `\n    suspend fun reqSupReqInQueueExInChunk${methodName}(${paramName}: ${reqClassName}): Boolean {\n`;
        ...
        if (reqClassName && includeServerSendData) {
            const methodName = getServiceMethodName(baseName);
            const paramName = baseName.toLowerCase() + 'Req';
            code += `\n    suspend fun reqSendData${methodName}(${paramName}: ${reqClassName}) {\n`;
